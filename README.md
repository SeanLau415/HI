...
### Debian root bootstrap + upload config

If you want the safest path on a Debian VPS as `root`, bootstrap first:

```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/SeanLau415/HI/main/bootstrap_debian_root.sh)"
```

This command will:
- install `git` and `python3`
- clone or update the repo into `/root/HI`
- create a placeholder `/root/HI/config.yaml`
- install Python dependencies
- create the `monitoring-worker` systemd service

Then upload your own `config.yaml` to:

```text
/root/HI/config.yaml
```

The placeholder file is intentionally blocked from starting, so you will not
accidentally run the worker with example settings.

After that, start the worker with:

```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/SeanLau415/HI/main/start_debian_root.sh)"
```

To watch logs:

```bash
journalctl -u monitoring-worker -f
```
